...
class Notifier {
  constructor() {
    ...
    this.options = {
      ...
      duration: 4000,
      bigPopup: false,
      packagedLibRendererDirectory: '@ch3yne/electron-notifier/lib/renderer',
    }
    // 暂存待展示的通知队列
    this.queue = []
    // 已展示的通知队列
    ...
    this.resolveHtmlPath = this.getHtmlPath()
  }
  ...
  listenRendererMessage() {
    ...
    electron.ipcMain.on('notifier.beep', async (event, ...args) => {
      const option = args[0]
      this.beep(option)
      event.returnValue = true
    })
  ...
  getHtmlPath() {
    ...
    //     __dirname,
    //     this.options.packagedLibRendererDirectory,
    //     'notification.html'
    //   )}`
    // }
    // return `file://${path.resolve(__dirname, '../renderer/notification.html')}`
    return path.join(__dirname, '../renderer/notification.html')
  }
  ...
  mergeOption(options) {
    ...
    ;['icon', 'autoClose', 'duration', 'silent', 'bigPopup'].forEach((prop) => {
      notifyOptions[prop] = ensureNotUndefined(
        notifyOptions[prop],
        this.options[prop]
      )
    })
  ...
  notify(options) {
    ...
    let windowOptions = {
      ...
      x:
        this.primaryDisplay.workArea.width +
        this.primaryDisplay.workArea.x -
        this.options.width -
  ...
  beep() {
    ...
    if (!this.beepWindow) {
      this.beepWindow = new BrowserWindow(windowOptions)
      this.beepWindow.loadFile(this.resolveHtmlPath)
      this.beepWindow.webContents.on('did-finish-load', () => {
        this.beepWindow.webContents.send('notifier.beep')
      })
    }
    this.beepWindow.webContents.send('notifier.beep')
  }
  ...
  showCurrent(notification) {
    ...
    let notificationY =
      this.primaryDisplay.workArea.height + this.primaryDisplay.workArea.y
    ...
    notificationWindow.loadFile(this.resolveHtmlPath)
